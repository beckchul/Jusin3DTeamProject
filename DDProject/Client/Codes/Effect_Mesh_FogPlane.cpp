...
CEffect_Mesh_FogPlane::CEffect_Mesh_FogPlane(LPDIRECT3DDEVICE9 pGraphicDev)
	:CEffect_Mesh(pGraphicDev)
{
}

CEffect_Mesh_FogPlane::CEffect_Mesh_FogPlane(const CEffect_Mesh_FogPlane& rhs)
	: CEffect_Mesh(rhs)
...
CEffect_Mesh_FogPlane::~CEffect_Mesh_FogPlane(void)
{

}

CEffect_Mesh_FogPlane* CEffect_Mesh_FogPlane::Clone(void)
{
	return new CEffect_Mesh_FogPlane(*this);
}

HRESULT CEffect_Mesh_FogPlane::Ready_GameObject(void)
{
...
int CEffect_Mesh_FogPlane::Update_GameObject(const float & fTimeDelta)
{
...
void CEffect_Mesh_FogPlane::Render_GameObject(LPD3DXEFFECT pArgEffect, _uint uPassIdx)
{
...
HRESULT CEffect_Mesh_FogPlane::Add_Component(void)
{
	...
	// For.Mesh
	AddComponent(m_pStaticMeshCom, Engine::CStaticMesh*, SCENE_STAGE, L"Com_Mesh_FogPlane", Engine::CComponent::COM_STATIC, L"Com_Mesh");
...
HRESULT CEffect_Mesh_FogPlane::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect
...
CEffect_Mesh_FogPlane* CEffect_Mesh_FogPlane::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CEffect_Mesh_FogPlane* pInstance = new CEffect_Mesh_FogPlane(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MessageBox(0, L"CEffect_Mesh_FogPlane Created Failed", nullptr, MB_OK);
		pInstance->Release();
	}
	return pInstance;
}

void CEffect_Mesh_FogPlane::Free(void)
{
	Safe_Release(m_pTextureMaskCom);
	Safe_Release(m_pTextureFaderCom);

	CEffect_Mesh::Free();
	cout << "CEffect_Mesh_FogPlane 소멸!" << endl;
}
