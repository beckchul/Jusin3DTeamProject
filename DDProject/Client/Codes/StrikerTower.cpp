...
CStrikerTower::CStrikerTower(LPDIRECT3DDEVICE9 pGraphicDev)
: CTower(pGraphicDev)
, m_bIsCharging(FALSE)
{
}

CStrikerTower::~CStrikerTower()
{
}

HRESULT CStrikerTower::Ready_GameObject()
{
	...
	if (FAILED(Add_Component()))
		return E_FAIL;
	m_tInfo = INFO(200, 200, 0, 0, 400, false, 0.f, 0.f, 0.f);

	m_pTransformCom->Set_Information(CTransform::INFO_SCALE, &_vec3(0.f, 0.f, 0.f));

	m_tTargetCheck.fRength = 800.f;
...
int CStrikerTower::Update_GameObject(const float & fTimeDelta)
{
...
void CStrikerTower::FirstRender_GameObject(_bool bTrue)
{
	if(!bTrue)
...
void CStrikerTower::Render_GameObject(LPD3DXEFFECT pArgEffect, _uint uPassIdx)
{
	if (TRUE == m_bSummoning) // 타워 소환중
	{
...
HRESULT CStrikerTower::Add_Component(void)
{
	...
	// For.DynamicMesh
	AddComponent(m_pDynamicMeshCom, Engine::CDynamicMesh*, SCENE_STATIC, L"Com_Mesh_StrikerTower", Engine::CComponent::COM_STATIC, L"Com_Mesh");
...
HRESULT CStrikerTower::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect
...
void CStrikerTower::Change_State(void)
{
	if (TOWER_FIRE == m_eMainState)
...
void CStrikerTower::Create_Bullet(void)
{
	if (TOWER_FIRE == m_iRenderState)
	{
		// 노란색 게이지 모이는 이펙트 필요
		if (!m_bIsCharging)
		{
			...
			//Engine::CEffectMgr::GetInstance()->Add_Effect(L"Effect_StrikerTower_Charging", &_vec3(vPos.x, vPos.y + 34.f, vPos.z));
			m_bIsCharging = TRUE;
			...
			if (fDistance < 250.f)
			{
				CSoundMgr::GetInstance()->Play_Sound(L"Tower_DeadlyStrikerFire.ogg", Engine::CHANNEL_TOWER_2, 0.5f);
			}*/
		}
	}

	if (TOWER_FIRE == m_iRenderState  && m_pDynamicMeshCom->GetTrackPosition() >= 2.2f && m_pDynamicMeshCom->GetTrackPosition() <= 2.4f)
	{
		if (NULL == m_iAttackCount)
		{
			...
			_matrix* matHand = m_pDynamicMeshCom->Get_FrameMatrix("b_ArmMid");
			...
			pGameObject = CMissile::Create(m_pGraphicDev, vHandPos, vDir, MAGE_TOWER_STRIKER);
...
CStrikerTower* CStrikerTower::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CStrikerTower* pInstance = new CStrikerTower(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MessageBox(0, L"CStrikerTower Created Failed", nullptr, MB_OK);
		pInstance->Release();
	}
	return pInstance;
}

void CStrikerTower::Free(void)
{
	if (m_listChargingEffect.size() > 0)
	{
		list<Engine::CEffect*>::iterator iter_begin = m_listChargingEffect.begin();
		list<Engine::CEffect*>::iterator iter_end = m_listChargingEffect.end();
		for (; iter_begin != iter_end; ++iter_begin)
			(*iter_begin)->Set_Dead();
		m_listChargingEffect.clear();
	}
