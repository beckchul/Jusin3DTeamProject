...
HRESULT CLeftDoor::Ready_GameObject()
{
...
int CLeftDoor::FirstUpdate_GameObject(const float & fTimeDelta)
{
...
int CLeftDoor::Update_GameObject(const float & fTimeDelta)
{
...
void CLeftDoor::Render_GameObject(LPD3DXEFFECT pArgEffect, _uint uPassIdx)
{
...
void CLeftDoor::Play_Animation(const float& fTimeDelta)
{
...
HRESULT CLeftDoor::Add_Component(void)
{
	...
	// For.DynamicMesh
	AddComponent(m_pDynamicMeshCom, Engine::CDynamicMesh*, SCENE_STAGE, L"Com_Mesh_LeftDoor", Engine::CComponent::COM_STATIC, L"Com_Mesh");

	return NOERROR;
}

HRESULT CLeftDoor::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect
...
CLeftDoor * CLeftDoor::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	...
	{
		MessageBox(0, L"CLeftDoor Created Failed", nullptr, MB_OK);
		pInstance->Release();
	}
	return pInstance;
}
