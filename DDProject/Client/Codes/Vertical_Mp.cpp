...
CVertical_Mp::CVertical_Mp(LPDIRECT3DDEVICE9 pGraphicDev)
...
CVertical_Mp::~CVertical_Mp()
{
}

HRESULT CVertical_Mp::Ready_GameObject(void)
{
	...
	m_pTransformCom->Set_Information(CTransform::INFO_POSITION, &_vec3(SCALE_WINCX * 98.f, SCALE_WINCY * 669.f, 0.f));
...
int CVertical_Mp::Update_GameObject(const float & fTimeDelta)
{
...
void CVertical_Mp::Render_GameObject(LPD3DXEFFECT pArgEffect, _uint uPassIdx)
{
	...
	float fRatio = (float)pInfo->tUnitInfo.iMp / pInfo->tUnitInfo.iMpMax;
...
HRESULT CVertical_Mp::Add_Component(void)
{
...
HRESULT CVertical_Mp::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect
	...
	pEffect->SetVector("g_fColor", &_vec4(0.f, 0.f, 1.f, 0.f));

	return NOERROR;
}

CVertical_Mp * CVertical_Mp::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CVertical_Mp* pInstance = new CVertical_Mp(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MessageBox(0, L"CVertical_Mp Created Failed", nullptr, MB_OK);
		pInstance->Release();
	}
	return pInstance;
}

void CVertical_Mp::Free(void)
{
