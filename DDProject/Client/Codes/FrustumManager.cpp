...
IMPLEMENT_SINGLETON(CFrustumManager)
...
void CFrustumManager::Initialize_FrsutumMgr(void)
{
	// (-1, -1, 0) ~ (1, 1, 1)
	// 아래
...
void CFrustumManager::MakeFrustumPlane(D3DXMATRIX& matView, D3DXMATRIX& matProj)
{
...
bool CFrustumManager::IsPointInFrustum(D3DXVECTOR3 vecPos)
{
	for (int i = 0; i < 6; ++i)
	{
...
bool CFrustumManager::IsSphereInFrustum(D3DXVECTOR3 vecPos, float fRadius)
{
	for (int i = 0; i < 6; ++i)
	{
		float fDistance = D3DXPlaneDotCoord(&m_Plane[i], &vecPos);
...
void CFrustumManager::Free(void)
{
}
