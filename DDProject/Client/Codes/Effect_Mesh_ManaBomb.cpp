...
CEffect_Mesh_ManaBomb::CEffect_Mesh_ManaBomb(LPDIRECT3DDEVICE9 pGraphicDev)
	:CEffect_Mesh(pGraphicDev)
{
}

CEffect_Mesh_ManaBomb::CEffect_Mesh_ManaBomb(const CEffect_Mesh_ManaBomb& rhs)
	: CEffect_Mesh(rhs)
{
...
CEffect_Mesh_ManaBomb::~CEffect_Mesh_ManaBomb(void)
{

}

CEffect_Mesh_ManaBomb* CEffect_Mesh_ManaBomb::Clone(void)
{
	return new CEffect_Mesh_ManaBomb(*this);
}

HRESULT CEffect_Mesh_ManaBomb::Ready_GameObject(void)
{
...
int CEffect_Mesh_ManaBomb::Update_GameObject(const float & fTimeDelta)
{
...
void CEffect_Mesh_ManaBomb::Render_GameObject(LPD3DXEFFECT pArgEffect, _uint uPassIdx)
{
...
HRESULT CEffect_Mesh_ManaBomb::Add_Component(void)
{
	...
	// For.Mesh
	AddComponent(m_pStaticMeshCom, Engine::CStaticMesh*, SCENE_STATIC, L"Com_Mesh_ManaBomb", Engine::CComponent::COM_STATIC, L"Com_Mesh");

	return NOERROR;
}

HRESULT CEffect_Mesh_ManaBomb::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect
...
CEffect_Mesh_ManaBomb* CEffect_Mesh_ManaBomb::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CEffect_Mesh_ManaBomb* pInstance = new CEffect_Mesh_ManaBomb(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MessageBox(0, L"CEffect_Mesh_ManaBomb Created Failed", nullptr, MB_OK);
		pInstance->Release();
	}
	return pInstance;
}

void CEffect_Mesh_ManaBomb::Free(void)
{
	CEffect_Mesh::Free();
	cout << "CEffect_Mesh_ManaBomb 소멸!" << endl;
}
